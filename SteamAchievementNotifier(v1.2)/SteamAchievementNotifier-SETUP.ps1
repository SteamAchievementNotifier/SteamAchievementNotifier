...
Write-Host "Steam Achievement Notifier - Setup";
...
if ($user -eq $null) {
    ...
    Write-Host "Restarting script now - please try again and ensure credentials are correct. If error persists, please consult README.txt in SteamAchievementNotifier folder for help." -ForegroundColor Cyan;
    ...
    exit;
    } else { Write-Host "Welcome, $user! You have successfully installed Steam Achievement Notifier." -ForegroundColor Green;
...
sleep 5;
Copy-Item "${drive}:\Users\$env:username\Desktop\Steam Achievement Notifier.lnk" -Destination "${drive}:\Users\$env:username\AppData\Roaming\Microsoft\Windows\Start Menu\Programs\Startup";
