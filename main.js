...
if (!fs.existsSync(sanlocalappdata)) {
    console.log("\"Steam Achievement Notifier (V1.8)\" directory does not exist in %LOCALAPPPDATA%. Creating...")
...
ipcMain.on('notifywin', function(event, queueobj) {
   ...
   notifywin.once('ready-to-show', function() {
      notifywin.show();
      notifywin.webContents.send('notifymain', queueobj.achievement, queueobj.title, queueobj.desc, queueobj.icon, queueobj.screenshot, queueobj.percent);
      //notifywin.webContents.openDevTools({ mode: 'detach' })
   });
   ipcMain.on('notifywinstop', function() {
      notifywin.destroy();
   });
});
