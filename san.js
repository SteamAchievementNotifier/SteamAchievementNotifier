...
const shortcut = path.join(os.homedir(), "Desktop", "Steam Achievement Notifier (V1.7).lnk");
...
function ResetApp() {
    ...
    try {
        fs.rmSync(path.join(os.homedir(),"Desktop","Steam Achievement Notifier (V1.7).lnk"));
    ...
    try {
        fs.rmSync(path.join(process.env.APPDATA,"Microsoft","Windows","Start Menu","Programs","Startup","Steam Achievement Notifier (V1.7).lnk"));
...
function CreateDesktopShortcut() {
    ...
    if (fs.existsSync(shortcut)) {
        ...
        spawn("powershell.exe",["-NoExit","-Command",`$shell = New-Object -ComObject WScript.Shell; $shortcut = $shell.CreateShortcut('` + shortcut + `'); $shortcut.IconLocation = '` + path.join(process.env.LOCALAPPDATA,"Steam Achievement Notifier (V1.7)","img","sanlogo.ico") + `'; $shortcut.TargetPath = '` + path.join(process.env.PORTABLE_EXECUTABLE_FILE) + `'; $shortcut.Save(); $sc = "steam achievement notifier (V1.7)"; $txtinfo = (Get-Culture).TextInfo; $scfix = $txtinfo.ToTitleCase($sc); Rename-Item -Path "` + shortcut + `" -NewName "$scfix.lnk"`]);
        document.getElementById("desktop").checked = true;
    ...
    var startsan = setInterval(function() {
        ...
        for (var i = 0; i < url.playerstats.achievements.length; i++) {
            if (local.playerstats.achievements[i].achieved !== url.playerstats.achievements[i].achieved) {
                ...
                var percent = getpercent.achievementpercentages.achievements[i].percent;
