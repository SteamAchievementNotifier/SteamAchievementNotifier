...
const launcher = JSON.parse(fs.readFileSync(path.join(process.env.LOCALAPPDATA,"Steam Achievement Notifier (V1.8)","store","launcher.json")));
const gamestats = JSON.parse(fs.readFileSync(path.join(process.env.LOCALAPPDATA,"Steam Achievement Notifier (V1.8)","store","gamestats.json")));
...
function LoadLang() {
    ...
    if (config.lang == "english") {
        ...
        document.getElementById("ssoverlaylbl").innerHTML = "Speichern Sie Screenshots mit Overlay";
...
function RemoveApp() {
    ...
    if (resettype == "reset") {
        ...
        try {
            fs.rmSync(path.join(process.env.LOCALAPPDATA,"Steam Achievement Notifier (V1.8)","store","config.json"));
            console.log("%c\"config.json\" deleted in %localappdata%\\Steam Achievement Notifier (V1.8)\\store.", "color: seagreen; font-family: 'Bahnschrift'")
...
function GetPlayerName() {
    ...
    if (!apikey || !steam64id) {
        ...
        fetch(apiurl).then(response => response.json()).then((data) => {
            ...
            fs.writeFileSync(path.join(process.env.LOCALAPPDATA,"Steam Achievement Notifier (V1.8)","store","launcher.json"), JSON.stringify(launcher, null, 2));
...
async function StartSAN() {
    try {
        ...
        if (!gamestats.games[appid]) {
            ...
            fs.writeFileSync(path.join(process.env.LOCALAPPDATA,"Steam Achievement Notifier (V1.8)","store","gamestats.json"), JSON.stringify(gamestats, null, 2));
        }
            ...
            fetch(gpa).then(response => response.json()).then((data) => {
                    ...
                    function CheckIfRunning() {
                        if (running == true) {
                            ...
                            if (config.screenshot == "true" && config.ssoverlay == "true") {
                                ipcRenderer.send('img', notifytitle, notifydesc, notifyicon, gamename)
                            }
                ...
                function CheckCompletionViaAPI() {
                    if (gamestats.games[appid].completed == true) {
                        console.log("%cGame already complete!", "color: orange; font-family: 'Bahnschrift'");
                    } else {
                        ...
                        if (acharr.every((achievement) => achievement == 1) == true) {
                            console.log("%cGame Complete!", "color: yellow; font-family: 'Bahnschrift'");
                            ...
                            console.log("%cGame has not yet been completed", "color: orange; font-family: 'Bahnschrift'");
                        }
                    }
                }
