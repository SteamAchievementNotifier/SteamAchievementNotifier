...
function LoadLang() {
    ...
    if (config.lang == "english") {
        ...
        document.getElementById("testrare").innerHTML = "POKAŻ POWIADOMIENIE RZADKIE TESTOWE";
        ...
        document.getElementById("roundnesslbl").innerHTML = "OKRĄGŁOŚĆ:";
        ...
        document.getElementById("roundnesslblrare").innerHTML = "OKRĄGŁOŚĆ:";
        ...
        if (running == true) {
            ...
            setTimeout(function() {
                ...
                if (queue.length == 0) {
                    ...
                    console.log("Queue Position: " + queue.length, "color: grey; font-family: 'Bahnschrift'");
                    //console.log(queue)
                }
...
if (config.soundmode == "file") {
    document.getElementById("soundmodediv").innerHTML = "<img src='./icons/audiofile.svg'>Sound Mode: <span style='color: lightskyblue'>" + file + "</span>";
    ...
    document.getElementById("soundmodediv").innerHTML = "Sound Mode: FOLDER";
...
function CheckSoundSource() {
    ...
    if (sounddir[random].includes(".wav") || sounddir[random].includes(".mp3") || sounddir[random].includes(".flac") || sounddir[random].includes(".ogg") || sounddir[random].includes(".m4a") || sounddir[random].includes(".aiff") || sounddir[random].includes(".wma")) {
        ...
        if (counter >= 50) {
            ...
            //console.log(sounddir[random] + ": File is not a valid sound file. Choosing new file...");
            CheckSoundSource();
            setTimeout(function() {
                counter = 0;
            }, 2000);
...
function CheckRareSoundSource() {
    ...
    if (raresounddir[random].includes(".wav") || raresounddir[random].includes(".mp3") || raresounddir[random].includes(".flac") || raresounddir[random].includes(".ogg") || raresounddir[random].includes(".m4a") || raresounddir[random].includes(".aiff") || raresounddir[random].includes(".wma")) {
        ...
        if (counter >= 50) {
            ...
            //console.log(raresounddir[random] + ": File is not a valid sound file. Choosing new file...");
...
function GetPlayerName() {
    ...
    if (!apikey || !steam64id) {
        ...
        //statustt = `<div style="font-family: 'Bahnschrift'; font-size: 12px; font-weight: bold; color: red;">UNLINKED</div><div>Please ensure that your<br>API Key/Steam64ID are correct.</div>`;
    } else {
        fetch(apiurl).then(response => response.json()).then((data) => {
            ...
            //statustt = `<div style="font-family: 'Bahnschrift'; font-size: 12px; font-weight: bold; color: green;">LINKED</div><div>Connected to Steam Web API.</div>`;
        }).catch(error => {
            document.getElementById("username").innerHTML = nouser;
            document.getElementById("username").style.color = "red";
            document.getElementById("statusdot").src = "./icons/dot_red.svg";
...
function CreateDesktopShortcut() {
    ...
    if (fs.existsSync(shortcut)) {
        ...
        //spawn("powershell.exe",["-NoExit","-Command",`$shell = New-Object -ComObject WScript.Shell; $shortcut = $shell.CreateShortcut('` + shortcut + `'); $shortcut.IconLocation = '` + path.join(process.env.LOCALAPPDATA,"Steam Achievement Notifier (V1.8)","img","sanlogo.ico") + `'; $shortcut.TargetPath = '` + "C:/Windows/system32/notepad.exe" + `'; $shortcut.Save(); $sc = "steam achievement notifier (V${thisver})"; $txtinfo = (Get-Culture).TextInfo; $scfix = $txtinfo.ToTitleCase($sc); Rename-Item -Path "` + shortcut + `" -NewName "$scfix.lnk"`]);
        document.getElementById("desktopbox").checked = true;
    }
}
...
function GetNotifyStyle() {
    ...
    if (document.getElementById("customiserstyledropdown").value == "default") {
        ...
        //notifysrc = "./notify/default/main/default.html";
        paused = false;
        document.getElementById("pause").src = "./icons/pause_white.svg";   
        ...
        //notifysrc = "./notify/xbox/main/xbox.html";
        paused = false;
        document.getElementById("pause").src = "./icons/pause_white.svg";
        ...
        //notifysrc = "./notify/playstation/main/playstation.html";
        paused = false;
        document.getElementById("pause").src = "./icons/pause_white.svg";
        ...
        //notifysrc = "./notify/windows/main/windows.html";
        paused = false;
        document.getElementById("pause").src = "./icons/pause_white.svg";
...
function GetNotifyStyleRare() {
    ...
    if (document.getElementById("customiserstyledropdownrare").value == "default") {
        ...
        //notifysrc = "./notify/default/rare/defaultrare.html";
        paused = false;
        document.getElementById("pauserare").src = "./icons/pause_white.svg";
        ...
        //notifysrc = "./notify/xbox/rare/xboxrare.html";
        paused = false;
        document.getElementById("pauserare").src = "./icons/pause_white.svg";
        ...
        //notifysrc = "./notify/playstation/rare/playstationrare.html";
        paused = false;
        document.getElementById("pauserare").src = "./icons/pause_white.svg";
        ...
        //notifysrc = "./notify/windows/rare/windowsrare.html";
        paused = false;
        document.getElementById("pauserare").src = "./icons/pause_white.svg";
...
/*document.getElementById("customiserbgdropdown").addEventListener('change', function(event) {
    ReplayNotification();
    PauseNotification();
});*/
...
/*function ShowNotifyWin() {
    ...
    var audio = document.getElementById("audio");
    audio.play();
    ipcRenderer.send('notifywin', notifystyle, notifywidth, notifyheight, notifysrc);
    setTimeout(function() {
        ipcRenderer.send('notifywinstop');
        //running = false;
    }, config.displaytime * 1000 + 1000);
...
/*document.getElementById("iconselect").onchange = function(selection) {
    var file = selection.target.files[0];
    config["icon"] = (file.path).replace(/\\/g,"/");
    fs.writeFileSync(path.join(process.env.LOCALAPPDATA,"Steam Achievement Notifier (V1.8)","store","config.json"), JSON.stringify(config, null, 2));
    GetBGType();
    ReplayNotification();
    PauseNotification();
...
function GetPlayer() {
    regkey.list([`HKCU\\SOFTWARE\\Valve\\Steam`], function(err, result) {
        player = result["HKCU\\SOFTWARE\\Valve\\Steam"].values.LastGameNameUsed.value;
...
function StartSAN() {
        ...
        fetch(gpa).then(response => response.json()).then((data) => {
            ...
            for (var i = 0; i < url.playerstats.achievements.length; i++) {
                if (local.playerstats.achievements[i].achieved !== url.playerstats.achievements[i].achieved) {
                    //console.log(url.playerstats.achievements[i])
                    ...
                    percentages.forEach(function(achievement) {
                        if (achievement.name == apiname) {
                            ...
                            //console.log(apiname + ": " + percent + "%");
                        }
                    });

                    var configpercent = parseFloat(config.rarity);
                    configpercent = Math.round(configpercent * 100) / 100;
                    //configpercent = configpercent.toFixed(2);
                    //console.log("Config Rarity: " + configpercent + " | Percent: " + percent)

                    //if (percent < config.rarity) {
    ...
    var checkgame = setInterval(function() {
        //console.log("checkgame is running");
...
function ToggleNoSteam() {
    function RestartSteam() {
        ...
        setTimeout(function() {
            spawn("powershell.exe",["-Command",`start '` + steampath + `/steam.exe'; Read-Host`]);
        }, 2500);
    }
