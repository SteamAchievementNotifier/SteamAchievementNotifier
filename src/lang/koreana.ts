export const translations = {
    ...
    tooltips: {
        ...
        usesanwatcher: `SAN의 새로운 게임 프로세스 감시 기능 활성화<br><br><span class="ttdesc">새로운 SANWatcher와 기존 자동 프로세스 추적 방식의 차이점은 다음과 같습니다.<br><br><div class="wrapper sanwatcher"><span class="hl">ON</span><ul><li>게임 설치 디렉터리에 저장된 모든 프로세스의 시작/종료 이벤트를 적극적으로 모니터링합니다</li><li>현재 게임을 해제하기 위해 게임 실행 파일을 미리 알고 있을 필요가 없습니다</li><li>게임 실행 전 런처의 동작은 달라질 수 있으므로 모든 Steam 게임과의 완전한 호환성은 보장되지 않습니다</li></ul></div><br><div class="wrapper sanwatcher"><span class="hl" style="color: firebrick;">OFF</span><ul><li>게임 실행 후 게임 설치 디렉터리에서 실행 중인 실행 파일을 검색합니다</li><li>현재 게임을 해제하려면 게임 실행 파일을 알고 있어야 합니다</li><li>게임 프로세스를 자동으로 확인할 수 없는 경우, 게임을 자동으로 해제하려면 사용자가 <span class="hl">Auto-Release Games</span>에 새 항목을 수동으로 추가해야 합니다</li></ul></div>`,
        releasewaittime: `현재 게임을 해제하기 전에 SANWatcher가 새 게임 프로세스가 생성될 때까지 얼마나 오래 기다릴지 설정합니다<br><br><span class="ttdesc">예를 들어 현재 게임이 실제 게임을 실행하기 전에 게임 실행용 런처 또는 설정 창을 먼저 실행하는 경우, 이 설정은 해당 런처 또는 설정 창이 닫힌 후 실제 게임 창이 생성될 때까지 SANWatcher가 기다릴 시간을 초 단위로 결정합니다<br><br>이 대기 시간 내에 새로운 활성 게임 프로세스가 감지되면 해제가 취소되고 대신 새로 활성화된 프로세스가 추적됩니다<br><br>💡 현재 게임이 해제를 기다리는 동안 <span class="hl">Game Display</span> 상자에서 게임 이름이 <span style="color: yellow;">노란색</span>으로 깜박입니다</span>`
    },
    update: {
