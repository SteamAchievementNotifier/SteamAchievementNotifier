...
function Show-Screenshot {
    ...
    $cheevlist = Invoke-RestMethod "https://api.steampowered.com/ISteamUserStats/GetPlayerAchievements/v0001/?appid=$appid&key=$apikey&steamid=${steam64ID}?__random=$random";
    $cheevapiname = $cheevlist.playerstats.achievements[$i].apiname;

    $getpercent = (Invoke-WebRequest "https://api.steampowered.com/ISteamUserStats/GetGlobalAchievementPercentagesForApp/v0002/?gameid=$appid&format=json" -UseBasicParsing | ConvertFrom-Json).achievementpercentages.achievements.Where({ $_.name -eq $cheevapiname }).percent;
    $percent = [Math]::Round($getpercent,1);
...
do {
    ...
    #$cheevapiname = $currentCheev.playerstats.achievements[$i].apiname;
